...
def fix_name(record: dict[str,str]) -> str:
    ...
    replacements = {# Entire names
                    ...
                    'American Armn Intl C': 'American Armn International College',         
    ...
    result = find_canonical_name(record)
    if result == '':
...
def two_year(name: str) -> bool:
    ...
    if re.match(r'\bJunior College\b', name, re.I):
        junior = True
    elif re.match(r'\bUniversity\b', name, re.I):
        junior = False
    elif re.match(r'\bVirginia Tech\b', name, re.I):
        junior = False
    else:
        jr_matches = ('Community', 'Technical College', 'Tech')
        for pattern in jr_matches:
            if re.match(r'\b'+pattern+r'\b', name, re.I):
                junior = True
...
def filter_schools(transfer_credits: list[dict[str,str]]) -> list[dict[str,str]]:
    ...
    for i in range(len(transfer_credits)-1, -1, -1):
        ...
        # Fix the name, even if this is not the first time we've seen the school.
        record['ORG NAME'] = fix_name(record)

        if school not in unique_schools:
            unique_schools.add(school)
            filtered_schools.append({'OrgCode': school,
                                     'College': record['ORG NAME'],
                                     # two_year depends on fix_name to fix ORG NAME
                                     'TwoOrFourYear': str(two_year(record['ORG NAME']))})
